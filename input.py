...
class MotionDetector(Input):
    def __init__(self, min_frames, max_frames, min_area, max_silent_frames, uploader=None, notifier):
...
#                cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 255, 0), 2)
                ...
                if self.moving_frames != 0:
                    ...
                    if self.silent_frames == self.max_silent_frames:
                        if self.moving_frames >= self.min_moving_frames:
                            ...
                            logging.debug("false alarm, sorry")
                        self.save_bg()
                        self.moving_frames = 0
                        vid_frames.clear()
                        avg_centroids.clear()
            ...
            self.notifier.notify("STATUS=Total frames {}".format(total_frames))
